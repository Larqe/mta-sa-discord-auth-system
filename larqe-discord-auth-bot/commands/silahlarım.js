...
module.exports = {
    name: "silahlarım",
    description: 'Eğer Hesabınızı Bağladıysanız Silahlarınızı Listeler',
    type: 1,
    options: [],
    run: async (client, interaction) => {
        try {
            ...
            if (characterResults.length > 0) {
                ...
                const [itemResults] = await dbConnection.query('SELECT * FROM items WHERE owner = ? AND itemID = 115', [characterId]);

                if (itemResults.length > 0) {
                    const weaponFields = itemResults.map(item => {
                        const [weaponId, weaponSerial] = item.itemValue.split(':');
                        const weaponName = weaponNames[weaponId] || `Bilinmeyen Silah (${weaponId})`;

                        return {
                            name: weaponName,
                            value: "```" + `Seri Numarası: ${weaponSerial}` + "```",
                            inline: false
                        };
                    ...
                    const embed = new EmbedBuilder()
                        .setColor("Green")
                        .setTitle(`${character.charactername} - Silahlar`)
                    ...
                    const embed = new EmbedBuilder()
                        .setColor("Red")
                        .setTitle("Uyarı")
                        .setDescription("Karakter Silahsızdır..")
                        .setTimestamp();
                    return interaction.reply({ embeds: [embed], ephemeral: true });
                }
            } else {
